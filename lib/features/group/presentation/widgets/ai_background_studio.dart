...
class _AIBackgroundStudioState extends ConsumerState<AIBackgroundStudio> {
  ...
  Future<void> _generatePreview() async {
    ...
    try {
      final result = await FirebaseFunctions.instance
          .httpsCallable('generateGroupBackground')
          .call<Map<String, dynamic>>({
            'groupId': widget.groupId,
            'prompt': prompt,
            'style': _selectedStyle,
          });
      final url = result.data['imageUrl'] as String?;
      if (url == null || url.isEmpty) {
        throw StateError('The image service returned no image.');
      }
      setState(() {
        _previewUrl = url;
      });
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(error.message ?? 'Could not generate the image.'),
          ),
        );
      }
    } catch (error) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Could not generate the image: $error')),
        );
      }
    } finally {
  ...
  Future<void> _applyBackground() async {
    ...
    try {
      await ref.read(databaseRepositoryProvider).updateGroupMeta(
        widget.groupId,
        {'backgroundImage': _previewUrl, 'theme': 'custom'},
      );
      if (mounted) {
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText:
                      'e.g., "A futuristic forest with bioluminescent plants"',
                    ...
                    Container(
                      ...
                      child: _isGenerating
                          ...
                          : Image.network(
                              ...
                              key: ValueKey(
                                _previewUrl,
                              ), // Force fresh reload on every new URL
                              fit: BoxFit.cover,
                              loadingBuilder:
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: _isGenerating || _isApplying
                      ? null
                      : _generatePreview,
